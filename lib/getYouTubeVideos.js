...
export async function getYouTubeVideos(playlistId) {
  ...
  if (failedRequestsCache.has(playlistId)) {
    console.warn(`Playlist ID "${playlistId}" previously returned 404. Skipping request.`);
    return []; // 빈 배열 반환
  }
  ...
  try {
    ...
    if (response.status === 404) {
      console.warn(`Playlist ID "${playlistId}" not found. Caching and skipping future requests.`);
